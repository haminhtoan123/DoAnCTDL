...
{
	if (a.data != 0)
		a.out += intToString(a.data) + " + ";
	for (int i = 0; i < a.countccgv; i++)
	{
		if (a.ec[i].heso>0)
		{
			a.out += " + " + intToString(a.ec[i].heso) + a.ec[i].ec;
		}
		else
		if (a.ec[i].heso != 0)
		{
...
X minus2X(X& a, X& b, int data_at) //a - b
{
	X kq;
	kq.out = a.out;
	kq.data = a.data - b.data*data_at;
	kq.ec = new ArrayOfx[10];
	for (int i = 0; i < a.countccgv; i++)
	{
		bool exist = false;
		for (int j = 0; j < b.countccgv; j++)
		{
			if (a.ec[i].ec.compare(b.ec[j].ec) == 0)
			{
				kq.ec[kq.countccgv].heso = a.ec[i].heso - b.ec[j].heso*data_at;
				kq.ec[kq.countccgv].ec = a.ec[i].ec;
				kq.countccgv++;
				exist = true;
			}
		}
		if (exist == false)
		{
			kq.ec[kq.countccgv] = a.ec[i];
			kq.countccgv++;
		}
	}
	// a=b
	for (int i = 0; i < b.countccgv; i++)
	{
		bool exist = false;
		for (int j = 0; j < a.countccgv; j++)
		{

			if (a.ec[j].ec.compare(b.ec[i].ec) == 0)
			{
				exist = true;
			}
		...
		if (exist == false)
		{
			kq.ec[kq.countccgv].heso = -b.ec[i].heso*data_at;
			kq.ec[kq.countccgv].ec = b.ec[i].ec;
...
// missing array -> struct X
void arrayfix(int **A,int n)// n xn
{

}
void NghiemTongQuat(int **A, int *B, int n)// A la mang nxn
{

	int* missing = new int[100];
	int count = 0;
	X* a = new X[n];// x0,x1,x2,x3
	for (int i = 0; i < n; i++)
	{
		if (A[i][i] == 0)
		{
			...
			count++;
			a[i].flag = 1;// missing
			a[i].ec = new ArrayOfx[n];//replace n with n
		}
		a[i].countccgv = 0;
	}
	for (int i = 0; i < n; i++)// create out
	{

		a[i].out = 'x';
		a[i].out += intToString(i + 1);
		a[i].out += " =";
	}
	int addchar = 223;
	for (int i = n - 1; i >= 0; i--)
	{
		...
		tinh.out = a[i].out;
		tinh.data = B[i];
		tinh.ec = new ArrayOfx[n];
		if (a[i].flag == 1)// if missing
		{
			a[i].countccgv++;
			a[i].ec[0].ec = char(++addchar);
			a[i].ec[0].heso = 1;
			//a[i].out = char(++addchar);
		}
		else
		{
